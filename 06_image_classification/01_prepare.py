...
base_dir = os.path.join(execution_path, 'images')   #base directory

CLS_1 = 'horse'
CLS_2 = 'lion'
TRAIN_RATIO = 0.7
VALID_RATIO = 0.2
DATA_DIR = r'./dataset'  #new directory

raw_no_of_files = {}
classes = [CLS_1, CLS_2]

number_of_samples = [(dir, len(os.listdir(os.path.join(base_dir, dir)))) for dir in classes]
print(number_of_samples)    #nuber of samples in horse and lion directories

#create directory for (train, valid, test) if it does not exist
if not os.path.exists(DATA_DIR):
    os.mkdir(DATA_DIR)

# directories for each dataset: train, valid, test
...
print('[INFO] File names validation...')
...
print(f'[INFO] Number of files in {CLS_1}: {len(cls_1_names)}')
print(f'[INFO] Number of files in {CLS_2}: {len(cls_2_names)}')

# indexes fo train, valid and test
...
print('[INFO] Copying files into proper directories...')
...
print(f'[INFO] Number of images in {CLS_1} in training set: {len(os.listdir(train_cls_1_dir))}')
print(f'[INFO] Number of images in {CLS_1} in validation set: {len(os.listdir(valid_cls_1_dir))}')
print(f'[INFO] Number of images in {CLS_1} in test set: {len(os.listdir(test_cls_1_dir))}')
print(f'[INFO] Number of images in {CLS_2} in training set: {len(os.listdir(train_cls_2_dir))}')
print(f'[INFO] Number of images in {CLS_2} in validation set: {len(os.listdir(valid_cls_2_dir))}')
print(f'[INFO] Number of images in {CLS_2} in test set: {len(os.listdir(test_cls_2_dir))}')